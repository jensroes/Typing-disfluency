...
model {
  ...
  for(s in 1:nS){
    int nBS = nB[s];
      ...
      n += 1;
      if(b == 1){
        lp_parts[1] = log_theta[1,1] + normal_lpdf(logy[s,b] | alpha[1] + delta[1] + RE[n], sigma);
        lp_parts[2] = log_theta[2,1] + normal_lpdf(logy[s,b] | alpha[1] + RE[n], sigma);
      }
      if(b > 1){
        lp_parts[1] = log_theta[1,2] + normal_lpdf(logy[s,b] | alpha[2] + delta[2] + phi*logy[s, b-1] + RE[n], sigma);
        lp_parts[2] = log_theta[2,2] + normal_lpdf(logy[s,b] | alpha[2] + phi*logy[s, b-1] + RE[n], sigma);
      }
    ...
    for(b in 1:nBS){
      ...
      if(b == 1){
        log_lik[n] = log_sum_exp(
 		      log_theta[1,1] + normal_lpdf(logy[s,b] | alpha[1] + delta[1] + RE[n], sigma), 
   		    log_theta[2,1] + normal_lpdf(logy[s,b] | alpha[1] + RE[n], sigma));
   		   theta_tilde = bernoulli_rng(theta[1]); 
         if(theta_tilde) { 
            y_tilde[n] = normal_rng(alpha[1] + delta[1] + RE[n], sigma);
         }
         else{
            y_tilde[n] = normal_rng(alpha[1] + RE[n], sigma);
        }
      }
      if(b > 1){      
	      log_lik[n] = log_sum_exp(
 		      log_theta[1,2] + normal_lpdf(logy[s,b] | alpha[2] + delta[2] + phi*logy[s, b-1] + RE[n], sigma), 
   		    log_theta[2,2] + normal_lpdf(logy[s,b] | alpha[2] + phi*logy[s, b-1] + RE[n], sigma));
   		  theta_tilde = bernoulli_rng(theta[2]); 
        if(theta_tilde) { 
          y_tilde[n] = normal_rng(alpha[2] + delta[2] + phi*logy[s, b-1] + RE[n], sigma);
        }
        else{
          y_tilde[n] = normal_rng(alpha[2] + phi*logy[s, b-1] + RE[n], sigma);
        }
      }
    }
  }
