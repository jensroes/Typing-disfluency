...
model {
  ...
  for(s in 1:nS){
    ...
    for(b in 1:nBS){ 
      ...
      if(b == 2){
        lp_parts[1] = log_theta[1,2] + normal_lpdf(logy[s,b] | alpha[2] + delta[2] + phi[1]*logy[s, b-1] + RE[n], sigmap_e);
        lp_parts[2] = log_theta[2,2] + normal_lpdf(logy[s,b] | alpha[2] + phi[2]*logy[s, b-1] + RE[n], sigma_e);
      }
      if(b > 2){
        lp_parts[1] = log_theta[1,2] + normal_lpdf(logy[s,b] | alpha[2] + delta[2] + phi[1]*logy[s, b-1] + gamma[1]*logy[s, b-2] + RE[n], sigmap_e);
        lp_parts[2] = log_theta[2,2] + normal_lpdf(logy[s,b] | alpha[2] + phi[2]*logy[s, b-1] + gamma[2]*logy[s, b-2] + RE[n], sigma_e);
      }
...
generated quantities{
  ...
  for(s in 1:nS){
    ...
    for(b in 1:nBS){
      ...
      if(b == 2){      
	      log_lik[n] = log_sum_exp(
 		      log_theta[1,2] + normal_lpdf(logy[s,b] | alpha[2] + delta[2] + phi[1]*logy[s, b-1] + RE[n], sigmap_e), 
   		    log_theta[2,2] + normal_lpdf(logy[s,b] | alpha[2] + phi[2]*logy[s, b-1] + RE[n], sigma_e));
   		  theta_tilde = bernoulli_rng(theta[2]); 
        if(theta_tilde) { 
          y_tilde[n] = normal_rng(alpha[2] + delta[2] + phi[1]*logy[s, b-1] + RE[n], sigmap_e);
        }
        else{
          y_tilde[n] = normal_rng(alpha[2] + phi[2]*logy[s, b-1] + RE[n], sigma_e);
        }
      }
      if(b > 2){
        log_lik[n] = log_sum_exp(
   		    log_theta[1,2] + normal_lpdf(logy[s,b] | alpha[2] + delta[2] + phi[1]*logy[s, b-1] + gamma[1]*logy[s, b-2] + RE[n], sigmap_e), 
     		  log_theta[2,2] + normal_lpdf(logy[s,b] | alpha[2] + phi[2]*logy[s, b-1] + gamma[2]*logy[s, b-2] + RE[n], sigma_e));
        theta_tilde = bernoulli_rng(theta[2]); 
        if(theta_tilde) { 
          y_tilde[n] = normal_rng(alpha[2] + delta[2] + phi[1]*logy[s, b-1] + gamma[1]*logy[s, b-2] + RE[n], sigmap_e);
        }
        else{
          y_tilde[n] = normal_rng(alpha[2] + phi[2]*logy[s, b-1] + gamma[2]*logy[s, b-2] + RE[n], sigma_e);
        }
      }
    }
  }
