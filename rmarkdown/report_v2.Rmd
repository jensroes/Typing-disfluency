...
**Hesitations in keyboard typing are indicative of higher level processes. The maximum fluency of a typist composing text or copying text is restricted by motor planning and execution. However, sometimes upstream processes have to catch up -- when deciding what to say next or reminding ourselves of the text we are copying -- meaning that the output speed decreases. These hesitations are typically referred to as disfluencies and are invaluable to develop an understanding of an individual's writing dynamics. This is because it allows insight into the information cascade that underlies language production from the mental generation of a message, into grammatical processing, and finally the generation and execution of motor codes. In particular disfluencies are indicators of process demands that arise on higher levels of mental representation [@christiansen2016now;@olive2014toward]; for example when preplanning syntactic dependencies [@roeser2019advance] or retrieving the lexical entry of a word or its spelling [@torrance2016adolescent]. This idea can be found in theoretical models of spoken language production [@bock2014syntactically], handwriting [@van1991handwriting] and keyboard typing [@hayes2012evidence]. At present there is no principled way to detect hesitations at the execution stage that can be considered process disfluencies. In this paper we present a series of statistical models that aim to capture this theoretical process as a combination of fluent and disfluent keystroke transitions and to produce by-participant estimates of typing performance.**
...
**To test the model that we describe in the following section we use data from two copy-tasks. Using a constrained writing context such as a copy task data rather than data from spontaneous text production was fundamental for our purposes. This is because, first, we can reduce the influence of higher level processes. While text production involves the generation of content and its linguistic encoding, high-level processes in copy-typing are constrained to reading. In other words, the cognitive source of longer keystroke intervals that are not related to the generation and execution of motor programs is less ambiguous. Second, in spontaneous text production there is a considerable variability between what participants write. In a copy task we can largely remove this variability.**
