...
We require two packages: (1) `rstan` to use R to interface with Stan for fitting Bayesian models; (2) `tidyverse` for data processing and visualisation.
...
The Stan code described in the manuscript was extended to fit more than one copy-task component (or generally condition) at once. The code is saved in the file "MoG.stan" (i.e. mixture of Gaussians) and can be loaded for data fitting using the `stan_model` function and assign it to the object `mog`. The "MoG.stan" file needs to available in the current working directory of $R$ or the file path needs to be indicated.
