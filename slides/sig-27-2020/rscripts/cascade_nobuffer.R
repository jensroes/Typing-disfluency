library(tidyverse)
library(grid)

casc <- tibble(stage = c(rep("Conceptual\nencoding",3), 
                         rep("Linguistic\nencoding",3), 
                         rep("Orthographic\nprocessing",3), 
                         rep("Motor\nplanning",3)),
               activity = rep("Planning", 12),
               start = c(0,  6,  10, 
                         6,  10, 14,
                         10, 14, 16,
                         14, 16, 18
                         )) %>%
  mutate(end = lead(start,3),
         end = ifelse(is.na(end), start + 2, end)) %>% ungroup() %>%
  mutate(item = c(rep(4,3),rep(3,3),rep(2,3), rep(1,3)),
         end = end - .05) %>%
  group_by(stage, activity) %>%
  mutate(chunk = 1:(n()),
         chunk = as.character(chunk - 1)) %>% ungroup() %>%
  mutate(chunk = ifelse(chunk == 0, "italic('n')", paste0("italic('n')~+~", chunk)),
         chunk = ifelse(start == 0, paste0("chunk~", chunk), chunk))
         
p_nobuffer <- ggplot(casc, aes(x = start, xend = end, 
                 y = reorder(stage,item),
                 yend = reorder(stage,item),
                 colour = activity)) + geom_segment(size = 12, show.legend = F) +
  geom_segment(aes(x = end, xend = end, 
                   y = item - .25, 
                   yend = item - .65),
                  show.legend = F, colour = "grey60", size = .25,
                 arrow = arrow(length = unit(0.2, "cm"),
                               type = "closed")) +
  geom_text(aes(x = (start + end) / 2 , label = chunk), 
                  size = 4, colour = "white", parse = TRUE) +
  #ggtitle("a. Idealised planning cascade") +
  scale_colour_viridis_d("", begin = .5) +
  scale_x_continuous(limits = c(0,22), breaks = 16,labels = "writing\nonset") +
  labs(x = "Time", y = "Planning\nstage") + theme_bw() +
  theme(axis.text.x = element_text(face = "italic", size = 14),
        axis.text.y = element_text(size = 14),
        axis.title.x = element_text(hjust = 0, size = 14),
        axis.title.y = element_text(size = 14, hjust = 1, angle = 360),
        axis.ticks = element_blank(),
        panel.grid = element_blank(),
        legend.position = "top",
        legend.justification = "right"); p_nobuffer

ggsave("slides/sig-27-2020/gfx/cascade_nobuffer.pdf", width = 8.25, height = 4.5)

